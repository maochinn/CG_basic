...
public:
		...
		:shader(shader), size(size), position(position)
	{
		...
		//generate VAO
		{
			int width = textures[0].size.x / 2;
			int height = textures[0].size.y / 2	;
	...
	void render(Shader* shader = nullptr, CubeMap* cubemap = nullptr)
	{
		if (shader == nullptr)
			shader = &(this->shader);
		...
		glUniform1i(glGetUniformLocation(shader->Program, "u_material.texture_displacement"), 0);
		glUniform1i(glGetUniformLocation(shader->Program, "u_use_height_map"), true);
		glUniform1f(glGetUniformLocation(shader->Program, "u_displacement_factor"), this->size.y);
		
		if (cubemap)
		{
			cubemap->bind(1);
			glUniform1i(glGetUniformLocation(this->shader.Program, "u_material.texture_cubemap"), 1);
		}
		...
		glUniform3f(glGetUniformLocation(shader->Program, "u_material.ambient"), 0.2f, 0.2f, 0.3f);
		glUniform3f(glGetUniformLocation(shader->Program, "u_material.diffuse"), 0.5f, 0.5f, 0.6f);
		glUniform3f(glGetUniformLocation(shader->Program, "u_material.specular"), 0.1f, 0.1f, 0.1f);
		glUniform1f(glGetUniformLocation(shader->Program, "u_material.shininess"), 16.0f);
		glUniform2fv(glGetUniformLocation(shader->Program, "u_texture_size"), 1, &glm::vec2(this->textures[animation_idx].size)[0]);
		...
		else
		{
			...
			glDrawElements(GL_PATCHES, this->vao.elementAmount, GL_UNSIGNED_INT, 0);
		}
